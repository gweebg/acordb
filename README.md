...
![image.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/abe24f37-595a-4e06-a9a4-456e49fef9a8/image.png)
...
| --- | --- | --- | --- |
| User | /accounts/user/ | GET | Devolve a informação relativa ao utilzador da sessão atual. |
|  | /accounts/user/{id} | GET | Devolve a informação relativa ao utilzador com o id definido. |
|  | /accounts/statistics/ | GET | Devolve as estatísticas relativas a um utilizador. |
|  | /accounts/requests/ | GET | Devolve os pedidos de alteração realizados pelo utilizador da sessão atual. |
|  | /accounts/user/ | POST | Regista um utilizador novo através dos dados passados no body do pedido. |
|  | /accounts/genApiKey/ | POST | Gera uma chave de API para o utilizador da sessão atual. |
|  | /accounts/user/{email}/makeadmin/ | POST | Permite a transformação de um utilizador normal em administrador. |
|  | /accounts/user/{id}/ | PATCH | Permite a modificação de campos relativos ao utilizador. |
| Acórdãos | /acordaos/?{query_string} | GET | Pesquisa de acórdãos com os parametros passados à query string. |
|  | /acordaos/{id_acordao} | GET | Devolve os dados coorrespondentes ao acórdão do id especificado. |
|  | /acordaos/records/{id_acordao} | GET | Lista todas as versões do acórdão do id especificado. |
|  | /acordaos/record/{id_record} | GET | Devolve os dados relativos à versão especificada no URI. |
|  | /acordaos/tags/ | GET | Devolve todos os descritores registados na base de dados. |
|  | /acordaos/fields/ | GET | Devolve todos os campos que um acórdão pode ter. |
|  | /acordaos/changeRequest/{id_acordao} | GET | Devolve todos os pedidos de alteração feitos a um acordão. |
|  | /acordaos/changeRequests/ | GET | Devolve todos os pedidos de alterações realizados até à data. |
|  | /acordaos/ | POST | Cria um novo acórdão de acordo com o conteúdo passado no body. |
|  | /acordaos/changeRequest/{id} | POST | Cria um novo pedido de alteração para o acórdão especificado. |
|  | /acordaos/changeRequest/{id} | PATCH | Aceita um pedido de alteração. |
|  | /acordaos/changeRequest/{id} | DELETE | Rejeita um pedido de alteração. |
| Favoritos | /favorites/ | GET | Retorna a lista de favoritos do utilizador da sessão atual. |
|  | /favorites/{id} | GET | Retorna os dados relativos a um certo favorito. |
|  | /favorites/ | POST | Cria um novo favorito. |
|  | /favorites/{id} | PATCH | Atualiza a descrição de um dado favorito. |
|  | /favorites/{id} | DELETE | Remove um favorito. |
| Outros | /accounts/login/password/ | POST | Autentica um utilizador através do email e password. |
|  | /statistics/ | GET | Retorna estatísticas gerais relativas ao serviço. |